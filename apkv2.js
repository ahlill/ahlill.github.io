...
const jawaban =`Jawaban : \n
					...
					Headloss Belokan
					HL belokan 90 derajat = f*30*v^2/2g
					HL belokan 90 derajat = ${f.toFixed(ketelitian)}*30*${vk}/2${g}
					HL belokan 90 derajat = ${headloss90.toFixed(ketelitian)} \n
					HL belokan 45 derajat = f*16*v^2/2g
					HL belokan 45 derajat = ${f.toFixed(ketelitian)}*16*${vk}/2${g}
					HL belokan 45 derajat = ${headloss45.toFixed(ketelitian)} \n
					Headloss belokan total adalah = ${headloss_bel.toFixed(ketelitian)} \n
					Headloss Keran
					HL keran gerbang = f*8*(v^2)/2g
					HL keran gerbang = ${fket}*8*${vk}/2*${g}
					HL keran gerbang = ${headlossg.toFixed(ketelitian)} \n
					HL keran bola = f*340*(v^2)/2g
					HL keran bola = ${fket}*340*${vk}/2*${g}
					HL keran bola = ${headlossb.toFixed(ketelitian)} \n
					HL keran sudut = f*150*(v^2)/2g
					HL keran sudut = ${fket}*150*${vk}/2*${g}
					...
					HL minor total = ${headloss_e.toFixed(ketelitian)} + ${headloss_bel.toFixed(ketelitian)} + ${headloss_kt}
